...
class Centroid : public FunctionPass {
 public:
    ...
    virtual bool runOnFunction(Function &F) {
        ...
        //errs() << F.getName() << "\n";        
        C_vector Cv;
        unsigned CxW = 0, CyW = 0, CzW = 0, CcW = 0, W = 0;
        ...
        for(SmallVectorImpl<std::pair<BasicBlock *, BasicBlock *> >::iterator 
            ...
            CcW += w1 * getCallingNumber(I->first) + w2 * getCallingNumber(I->second);
        }
        //errs() << "CxW=" << CxW << "\tCyW=" << CyW << "\tCzW=" << CzW <<"\tW=" << W << "\n";
        ...
        Cv.w = W;
        C_program.push_back(std::make_pair(F.getName(), Cv));
//errs() << "Cx=" << Cv.x << "\tCy=" << Cv.y << "\tCz=" << Cv.z <<"\tw=" << Cv.w << "\tCc=" << Cv.c << "\n";
    ...
    /// Get the number of callings in a basic block,
    unsigned getCallingNumber(BasicBlock *bb);
